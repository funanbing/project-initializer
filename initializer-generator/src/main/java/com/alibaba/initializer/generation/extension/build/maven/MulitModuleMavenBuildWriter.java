...
public class MulitModuleMavenBuildWriter extends MavenBuildWriter {
    ...
    public void writeTo(IndentingWriter writer, MavenBuild build) {
        ...
        writeProject(writer, () -> {
            ...
            if (settings.getName().indexOf("start") > 0) {
    ...
    private void writeBom(IndentingWriter writer, BillOfMaterials bom) {
        writeElement(writer, "dependency", () -> {
            ...
            if (bom instanceof DependencyBillOfMaterials) {
                DependencyBillOfMaterials dbom = (DependencyBillOfMaterials) bom;
                writeSingleElement(writer, "type", dbom.getType());
                writeSingleElement(writer, "scope", dbom.getScope());
            } else {
